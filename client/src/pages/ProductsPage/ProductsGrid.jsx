  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <svg
          className={`w-4 h-4 text-black transition-transform ${open ? "rotate-90" : "rotate-90"}`}
          viewBox="0 0 24 24"
          fill="none"
        >
          <path
        ...
        >
          {options.map((opt) => (
            <li
              ...
              className={`px-4 py-2 cursor-pointer hover:bg-blue-50 ${opt === value ? "text-[#193495] font-semibold" : "text-black"}`}
            >
              {opt}
            </li>
          ))}
...
const Pagination = ({ currentPage, totalPages, onPageChange }) => {
  ...
  // Show at most 4 page numbers + "..." if many
  const visiblePages =
    totalPages <= 5
      ? pages
      : currentPage <= 3
  ...
  return (
    <nav className="flex items-center gap-4" aria-label="Pagination">
      {/* Active circle highlight on current page */}
      <div className="flex items-center gap-4">
        {visiblePages.map((page, i) =>
          page === "..." ? (
            ...
            >
              {page === currentPage && (
                <span className="absolute inset-0 rounded-full border-2 border-[#193495]"></span>
              )}
              {page}
            </button>
          ),
