...
export default function AdminCategories() {
  const [categories, setCategories] = useState([
    { id: "CAT-01", name: "Smartwatches", items: 45 },
    { id: "CAT-02", name: "Accessories", items: 80 },
    { id: "CAT-03", name: "Chargers", items: 25 },
  ]);
  ...
  const handleSave = (e) => {
    e.preventDefault();
    alert(`Category ${modalState.type === "edit" ? "updated" : "added"} successfully!`);
    closeModal();
  };

  const handleDelete = () => {
    alert(`Category ${modalState.data?.id} deleted successfully!`);
    closeModal();
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      <div className="flex items-center justify-between">
        <h1 className="text-[28px] font-['Alegreya_Sans:Bold',sans-serif] text-[#0c1950]">Manage Categories</h1>
        <button 
        ...
        <table className="w-full text-left border-collapse min-w-[600px]">
          <thead>
            <tr className="bg-gray-50/80 border-b border-gray-100 text-[13px] font-['Lato:Bold',sans-serif] text-gray-500 uppercase tracking-wider">
              ...
              <th className="py-4 px-6 text-right rounded-tr-[16px]">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 text-[15px] font-['Lato:Regular',sans-serif] text-[#0c1950]">
            {categories.map((c) => (
              <tr key={c.id} className="hover:bg-gray-50/60 transition-colors">
                <td className="py-4 px-6 text-gray-500 font-mono text-[13px]">{c.id}</td>
                <td className="py-4 px-6 font-['Lato:SemiBold',sans-serif] text-[#193495]">{c.name}</td>
      ...
      <AdminModal
        ...
        onClose={closeModal}
        title={modalState.type === "edit" ? "Edit Category" : "Add New Category"}
        maxWidth="max-w-md"
      >
        <form onSubmit={handleSave} className="flex flex-col gap-5">
          <div>
            <label className="block text-[14px] font-['Lato:Bold',sans-serif] text-[#0c1950] mb-1.5">Category Name</label>
            <input 
              type="text" 
      ...
      >
        <div className="flex flex-col gap-4">
          <p className="text-[15px] font-['Lato:Regular',sans-serif] text-gray-600">
            Are you sure you want to delete <span className="font-['Lato:Bold',sans-serif] text-[#0c1950]">"{modalState.data?.name}"</span>? 
